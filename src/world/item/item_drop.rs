...
pub enum ItemDrop {
	...
	ConstantAmount { item: Item, amount: u8 }, // Always get the amount of items.
	RangedRandomAmount { item: Item, amount_range: RangeInclusive<u8> }, // Get a random amount in the amount range.
}

impl ItemDrop {
	/// Get a tuple containing the item and a random/constant amount in the ItemDrop range.
	pub fn roll(&self) -> (Item, u8) {
		match self {
